...
function FormNav() {
  return (
    <div className="navLink w-full flex flex-row gap-0 justify-between rounded-lg border-2 border-blueThree">
      ...
      <NavLink to={"/signin"} className={({ isActive }) => (isActive ? 'bg-blueThree rounded-md w-full px-8 py-4 font-bold text-white' : ' font-semibold w-full px-8 py-4')}>SignIn</NavLink>
    </div>
  )
}
